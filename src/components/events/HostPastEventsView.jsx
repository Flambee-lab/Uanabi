...
export default function HostPastEventsView({ events, onSelectEvent }) {
  if (events.length === 0) {
    return (
      <UanabiProfilePage
      ...
      >
        ...
        <p className="type-body-muted mx-auto mt-2 max-w-md">
          Cuando finalice un evento, vas a poder revisar el historial y cerrar patrocinios desde
          acá.
        </p>
      </UanabiProfilePage>
    )
  ...
  return (
    <UanabiProfilePage innerClassName="max-w-3xl">
      ...
      <p className="type-small mt-1 text-muted-foreground">
        Historial y cierres de patrocinio · {events.length} evento
        {events.length !== 1 ? 's' : ''}
      </p>
